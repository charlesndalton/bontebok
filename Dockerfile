...
WORKDIR /tmp/bontebok
COPY Cargo.toml /tmp/bontebok
COPY Cargo.lock /tmp/bontebok
...
RUN echo 'fn main() {}' >> /tmp/bontebok/dummy.rs
...
COPY --from=cargo-build /tmp/bontebok/target/x86_64-unknown-linux-musl/release/bontebok /
WORKDIR /
